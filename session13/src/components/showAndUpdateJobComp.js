...
const ShowAndUpdate = (props) => {
  ...
  const handleClickChange = () => {
    jobIsUpdating && updateNameJob(jobID, jobNameUpdate);
    setIsUpdating(jobID);
  };
    ...
    <div id="showAndUpdateComp">
      {jobIsUpdating ? (
        <input
          ...
          placeholder={jobName}
          value={jobNameUpdate}
          onChange={handleUpdateCurrentEl}
        />
      ) : (
        <span id="detailSpan">{jobName}</span>
      )}
      <img
        id="delete"
        ...
        onClick={handleClick}
      />
      <button onClick={handleClickChange} id="update-Btn">
        update
      </button>
    </div>
  );
