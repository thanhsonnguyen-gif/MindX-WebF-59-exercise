...
const insertReport = async (infoObject) => {
  const reportInserted = await db.reports.insertOne(infoObject);
  return reportInserted;
};

const countingReports = async () => {
  const numReports = await db.reports.countDocuments();
  return numReports;
};
