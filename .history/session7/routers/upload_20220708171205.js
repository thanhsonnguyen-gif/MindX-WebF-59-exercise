...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, callback) {
    ...
    //   suffix +
    //   file.originalname;
    req['fileAttach'] = `${path.join(__dirname, "../assets")}/`
...
router.post("/upload", AuthMdw, requiteAdminForReport, cpUpload, (req, res) => {
...
router.put("/upload", AuthMdw, requiteAdminForReport, async (req, res) => {
  ...
  if (report == null) {
    res
      .status(403)
      .send("databases isn't updated, input id of report may be incorrect or belong another person");
  }
});
