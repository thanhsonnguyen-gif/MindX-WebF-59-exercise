...
router.put("/upload", AuthMdw, requiteAdminForReport, async (req, res, next) => {
  ...
  const infoNeedUpdate = req.body;
    ...
    isReportOfCurrentUser.forEach((el)=> {
      if(el==true){
        
          const report = await UploadCtrl.updateReportById(id, infoNeedUpdate);
          if (!report) {
            throw new Error("Database isn't updated, check id of report form request");
          }
          const infoReportUpdated = await UploadCtrl.getReportById(id);
          res.json(infoReportUpdated);
            
      }else {
            res.status(403).send("the report is owned by another person");
    }});
});
