const { findByUsername, insertPasswordUser } = require("../databases/userData");
...
const register = async (_id, password) => {
  ...
  if (!userExisted) {
    throw new Error("User is not existed, connecting the school to get the information back");
  }
  //Step 2: Encrypt the password
  const { salt, hashedPassword } = EncryptPassword(password);
  //Step 3: Store inside databases
  const infoUserAfterRegister = await insertPasswordUser({
    salt: salt,
    hashedPassword: hashedPassword,
  });
  return infoUserAfterRegister;
};
